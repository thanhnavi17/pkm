...
<head>
<meta charset="UTF-8">
<title>Danh sách Move</title>
</head>
<body>
<c:url value="/admin/move/search" var="moveSearch" />
	<c:url value="/admin/move/update" var="moveUpd" />
	<c:url value="/admin/move/del" var="moveDel" />
	<div class="content-wrapper">
		...
		<section class="content-header">
			<div class="container-fluid">
				<div class="row mb-2">
					<div class="col-sm-6">
						<h1 class="pageModule" data-module="dsPkmMove">Move</h1>
					</div>
					<div class="col-sm-6">
		...
		<section class="content">
			<div class="container-fluid">
				<s:form action="${moveSearch }" method="post"
					modelAttribute="move" acceptCharset="utf-8">
					<div class="row">
				...
				<div class="row">
					<div class="col-12">
						<div class="card">
							...
							<!-- /.card-header -->
							<div class="card-body">
								<table id="pkmMove" class="table table-bordered table-hover">
									<thead>
										<tr>
											<th>Move</th>
											<th>Type</th>
									...
									<tbody>
										<c:if test="${not empty lstMove }">
											<c:forEach var="move" items="${lstMove }">
												<tr>
													<td>${move.moveName }</td>
													<td>${move.objType.type }</td>
													<td>${move.objCategory.moveType }</td>
													<td>${move.power }</td>
													...
													<td>${move.effect }</td>
													<td>${move.proc }</td>
													<td><a class="btnSua" style="display:none;" href="${moveUpd }/${move.moveID}">Sửa</a> <a
														onclick="return confirm('Bạn có chắn chắn muốn xoá pokemon này không ?');"
														href="${moveDel}/${move.moveID}" class="btnXoa" style="display:none;">Xoá</a></td>
												</tr>
											</c:forEach>
										</c:if>
									</tbody>
